...
ui {
	...
	steps := []step{
		...
		{expect: env._T(fmt.Sprintf("%s  /5", msgCream), "   - \x97\x97\x97\x97\x97\x94 +   "), inev: env._Key('1')},
		{expect: env._T(fmt.Sprintf("%s0", msgCredit), fmt.Sprintf(msgInputCode, "1")), inev: env._MoneyAbort},
		{expect: "", inev: Event{}}, // MoneyKeyAbort -> StateFrontEnd
	}
	uiTestWait(t, env, steps)
}
